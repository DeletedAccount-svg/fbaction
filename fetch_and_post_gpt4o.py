...
BLUESMINDS_API_KEY = os.environ["BLUESMINDS_API_KEY"]

BLUESMINDS_BASE_URL = "https://api.bluesminds.com"
...
def parse_json(raw: str) -> dict | None:
    """
    Robustly extract a JSON object from GPT-4o's response.
...
def groq_generate_caption_and_prompt(title, article_text, source="Google News"):
    """Use GPT-4o via BluesMinds API to generate a Facebook caption, image prompt, and overlay headline."""
    print("Asking GPT-4o (BluesMinds) to write caption, image prompt, and headline...")
    ...
    try:
        ...
        resp = requests.post(
            f"{BLUESMINDS_BASE_URL}/v1/chat/completions",
            headers=headers, json=body, timeout=30
        )
        resp.raise_for_status()
        raw = resp.json()["choices"][0]["message"]["content"].strip()
        print(f"  GPT-4o raw response preview: {raw[:200]}...")
        ...
        print(f"  GPT-4o caption error: {e}")
...
def generate_image(prompt):
    ...
    try:
        ...
        if not image_data:
            ...
            print(f"  ✅ Image received! ({len(raw_bytes):,} bytes)")
...
if __name__ == "__main__":
    print(f"Starting AI News Poster (GPT-4o via BluesMinds) — {datetime.now()}")
    posted = load_posted()
    ...
    # Fetch actual article content for better GPT-4o context
    article_text = fetch_article_content(article["url"])
    if not article_text:
        article_text = article.get("description", "")

    # GPT-4o generates the caption, image prompt, AND overlay headline
